...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    return NextResponse.json(
      { error: { code: 'INTERNAL_ERROR', message: error instanceof Error ? error.message : 'Failed to fetch guide' } },
      { status: 500 }
    )
  }
}
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const validated = guidePatchSchema.parse(body)
    const updates = {
      ...validated,
      ...
      file_storage_path: validated.file_storage_path === '' ? null : validated.file_storage_path,
      file_name: validated.file_name === '' ? null : validated.file_name,
      category: validated.category === '' ? null : validated.category,
    ...
    console.error('[Admin Guides PATCH]', error)
    return NextResponse.json(
...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    console.error('[Admin Guides DELETE]', error)
    return NextResponse.json(
